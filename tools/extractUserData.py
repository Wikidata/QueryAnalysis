...
for i in xrange(1, 2):
    with open("QueryProcessedOpenRDF" + "%02d" % i + ".tsv") as p, open("QueryCnt" + "%02d" % i + ".tsv") as s, open(
                            "userData/QueryProcessedOpenRDF" + "%02d" % i + ".tsv", "w") as user_p, open(
                        "userData/QueryCnt" + "%02d" % i + ".tsv", "w") as user_s:
