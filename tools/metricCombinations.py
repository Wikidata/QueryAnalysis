...
parser = argparse.ArgumentParser(
    description="This script generates a table that maps how often each "
    + "metric entry is used in combination with each other.")
...
parser.add_argument("metric", type=str,
                    help="The metric that should be matched")
parser.add_argument("month", type=str, help="The month for which the ranking "
                    + "should be generated.")
...
pathBase = utility.addMissingSlash(args.monthsFolder) \
        + utility.addMissingSlash(args.month) \
        + utility.addMissingSlash(metric + "_Combinations")
...
class combinationsHandler:
    ...
    def write(self):
        writeOut(pathBase + "Full_Month_" + metric
                 + "_Combinations.tsv",
                 self.monthlyFieldValues, self.monthlyData)
        for day in self.allDailyFieldValues:
            writeOut(pathBase + "Day_" + str(day)
                     + "_" + metric + "_Combinations.tsv",
