...
parser = argparse.ArgumentParser(
    description="This script lists the top N query types with their "
    + "respective user agents and some example queries.")
...
parser.add_argument("--numberOfQueryTypes", "-n", default=10, type=int,
                    help="The top n query types to list.")
parser.add_argument("--queryTypeRanking", "-q",
                    default="Full_Month_QueryType_Ranking.tsv", type=str,
                    help="The file in the QueryType-Folder to use as the "
                    + "ranking.")
...
monthPath = utility.addMissingSlash(args.monthsFolder) \
...
# grep QueryProcessedOpenRDFXX.tsv for these queryTypes and the 
# respective userAgents and create a directory for
    ...
    def writeOut(self):
        for queryType, userAgentCountDict in self.queryTypeUserAgentCombinationsCount.iteritems():
            for userAgent, valueDict in userAgentCountDict['userAgent'].iteritems():
                path = pathBase + queryType + "/" + str(valueDict['count']) \
                        + "_" + str(userAgent).replace('/', 'SLASH')[:100] + "/"
                ...
                if not os.path.exists(path):
                    ...
                    with open(path + "info.txt", "w") as info_file:
                        ...
                        toWrite += "\n#ExistingToolNames: " + ', '.join(
                            self.queryTypeUserAgentCombinationsCount[queryType]['userAgent'][userAgent]['existingToolNames'])
                        toWrite += "\n#Rank: " + str(
                            self.queryTypeUserAgentCombinationsCount[queryType]['rank'])
                ...
                for queryType, userAgentCountDict in self.queryTypeUserAgentCombinationsCount.iteritems():
                    for userAgent, valueDict in userAgentCountDict['userAgent'].iteritems():
                        combinations[valueDict['count']] = queryType + "/" \
                                + str(valueDict['count']) + "_" \
                                + str(userAgent).replace('/', 'SLASH')[:100]

                sortedCombinations = collections.OrderedDict(sorted(
                    combinations.items(), reverse=True))
